...
function onTouchMove(s, plt, ev) {
    ...
    if (s._isTouchEvent && ev.type === 'mousemove') 
        return;
    ...
    //else if (diff < 0 && currentTranslate < maxTranslate(s)) {
        ...
        if (s.resistance)
            ...
            s._currentTranslate = maxTranslate(s) + 1 - Math.pow(maxTranslate(s) - s._startTranslate - diff, s.resistanceRatio);
    }
